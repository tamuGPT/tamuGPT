...
def rank_results_with_llm(config, query_text, cleaned_result_array, cleaned_search_results, urls):
    rank_input_text = ""
    ...
    for i in range(len(rank_order_list)):
        ...
        ranked_urls.append(urls[rank - 1])

    return ''.join(ranked_order),ranked_urls
